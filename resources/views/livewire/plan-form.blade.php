        ...
        <div class="row">
            @foreach ($plans as $index => $plan)
                <div class="m bg-white rounded-lg card p-5 text-center col-md-4 mb-4">
                    ...
                    <div class="mb-4">
                        <label for="monthly_price-{{ $index }}" class="block text-gray-700">Monthly Price</label>
                        <input type="number" id="monthly_price-{{ $index }}" class="form-control"
                        ...
                        @if ($plan['name'] == 'starter' || $plan['name'] == 'pro' || $plan['name'] == 'enterprise')
                            <label for="annual_price-{{ $index }}" class="block text-gray-700">Annual
                                Price</label>
                        ...
                        @endif
                        @if ($plan['name'] == 'growth_pack' || $plan['name'] == 'scale_pack' || $plan['name'] == 'max_pack')
                            <label class="mt-2" for="">Allowed Agents</label>
                            <input type="number" id="price-{{ $index }}" class="form-control"
                                wire:model="plans.{{ $index }}.agents_per_month"
                                wire:change="updateAgents({{ $index }}, $event.target.value)" />
                        @endif
